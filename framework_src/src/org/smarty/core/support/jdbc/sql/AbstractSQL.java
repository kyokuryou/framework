...
public abstract class AbstractSQL<T> {
	...
	public T setLeftJoin(String join) {
		sql().leftJoin.add(join);
		return getSelf();
	}

	public T setRightJoin(String join) {
	...
	public static class SQLStatement {
		...
		private void sqlClause(SafeAppend builder, String keyword, List<String> parts, String open, String close, String conjunction) {
			if (!parts.isEmpty()) {
				if (!builder.isEmpty()) {
					builder.append(" ");
				}
		...
		private String selectSQL(SafeAppend builder) {
			...
			sqlClause(builder, "LEFT JOIN", leftJoin, "", "", " LEFT JOIN ");
			sqlClause(builder, "RIGHT JOIN", rightJoin, "", "", " RIGHT JOIN ");
		...
		private String callSQL(SafeAppend builder) {
			sqlClause(builder, "CALL", tables, "", "", "");
