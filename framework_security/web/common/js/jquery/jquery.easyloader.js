(function () {
    ...
    function loadSingle(name, callback) {
        ...
        var jsPath = easyloader.basePath + easyloader.jsBase;
        var cssPath = easyloader.basePath + easyloader.cssBase;

        if (easyloader.css && module['css']) {
            loadCss(cssPath + easyloader.theme + '/' + module['css'], function () {
                cssStatus = 'loaded';
        ...
        loadJs(jsPath + module['js'], function () {
            jsStatus = 'loaded';
            if (jsStatus == 'loaded' && cssStatus == 'loaded') {
                finish();
            }
        ...
        function loadMm() {
            if (mm.length) {
                ...
                if (easyloader.locale && doLoad == true && locales[easyloader.locale]) {
                    var url = easyloader.basePath + easyloader.jsBase + 'locale/' + locales[easyloader.locale];
                    runJs(url, function () {
                        finish();
                    });
                } else {
    ...
    easyloader = {
        ...
        locales:locales,
        basePath:'',
        jsBase:'/common/js/jquery/easyui/',
        cssBase:'/common/css/jquery/easyui/',
        theme:'default',
        css:true,
        locale:'zh_CN',
        timeout:2000,
        ...
        load:function (name, callback) {
            if (/\.css$/i.test(name)) {
                loadCss(easyloader.basePath + name, callback);
            } else if (/\.js$/i.test(name)) {
                loadJs(easyloader.basePath + name, callback);
            } else {
